...
class OblivionMirror:
        ...
        for i in range(len(vecs)):
            ...
            for j in range(i + 1, len(vecs)):
                ...
                if sim >= self.sim_threshold and _polarity(ei["text"]) != _polarity(ej["text"]):
                    anchors = self._anchors(ei["text"], ej["text"])
                    if anchors:
                        out.append({
                            "a": ei["text"], "b": ej["text"],
    ...
    @staticmethod
    def _anchors(text_a, text_b):
        """Topical anchors tying two statements together. Direct shared content
        words first; when the statements are pure paraphrases with no shared
        vocabulary, fall back to thesaurus synonym bridges ('lawyer~attorney')
        so reworded contradictions are not silently dropped."""
        wa, wb = _content_words(text_a), _content_words(text_b)
        ...
        try:
            ...
            if thesaurus.available():
                return thesaurus.get().bridges(wa, wb)[:6]
        except Exception:  # noqa: BLE001 - anchor fallback must never kill an audit
            pass
        return []
