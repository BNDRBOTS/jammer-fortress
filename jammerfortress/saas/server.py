...
LOGIN_FAIL_MAX = int(os.environ.get("JF_LOGIN_FAIL_MAX", "5"))
LOGIN_FAIL_WINDOW = 900  # seconds; failed-login lockout window per ip+email
...
class FortressServer(ThreadingHTTPServer):
    ...
    def __init__(self, addr, data_dir):
        ...
        self.kernel = Kernel(secret=self._secret(data_dir),
                             ...
                             mesh_capacity=int(os.environ.get("JF_MESH_CAPACITY", "100000")),
                             mesh_per_session=int(os.environ.get("JF_MESH_PER_USER", "1000")))
        self.kernel.boot(role="system_server", user="saas")
        self.rate = {}
        self.rate_lock = threading.Lock()
        self.login_fails = {}          # "ip|email" -> (count, window_start)
    ...
    # ---- failed-login throttle (anti password-spray, per ip+email) ---------
    def login_blocked(self, key):
        now = time.time()
        with self.login_lock:
            count, start = self.login_fails.get(key, (0, now))
            if now - start >= LOGIN_FAIL_WINDOW:
                self.login_fails.pop(key, None)
                return False
            return count >= LOGIN_FAIL_MAX

    def login_failed(self, key):
        now = time.time()
        with self.login_lock:
            count, start = self.login_fails.get(key, (0, now))
            if now - start >= LOGIN_FAIL_WINDOW:
                count, start = 0, now
            self.login_fails[key] = (count + 1, start)
            if len(self.login_fails) > 10000:
                self.login_fails = {k: v for k, v in self.login_fails.items()
                                    if now - v[1] < LOGIN_FAIL_WINDOW}

    def login_cleared(self, key):
        with self.login_lock:
            self.login_fails.pop(key, None)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _base_url(self):
        return compute_base_url(self.headers)
    ...
    def _login(self):
        ...
        email = (body.get("email") or "").strip()
        pw = body.get("password") or ""
        throttle_key = f"{self.client_address[0]}|{email.lower()}"
        if self.server.login_blocked(throttle_key):
            self.server.db.log("login_throttled", {"email": email.lower()})
            return self._json(429, {"error": "too many failed logins; wait 15 minutes"})
    ...
    def _webhook(self):
        ...
        if event_id and not self.server.db.mark_event(event_id):
            self.server.db.log("webhook_replay", {"event_id": event_id})
            return self._json(200, {"received": True, "action": "replay ignored"})
    ...
    def _ws_events(self, qs):
        ...
        while ws.open and time.time() < deadline:
            ...
            while cursor < len(snapshot):
                evt = snapshot[cursor]
                if event_visible(evt, user["id"]):
                    try:
                        ws.send_text(json.dumps(evt, default=str))
                    except RuntimeError:
                        return None
                cursor += 1
...
def compute_base_url(headers):
    """Base URL for Stripe redirect links. When JF_PUBLIC_URL is set it wins
    unconditionally, so a spoofed Host header can never steer checkout
    return URLs. Falls back to proxy headers for local/dev use."""
    env = os.environ.get("JF_PUBLIC_URL", "").strip()
    if env:
        return env.rstrip("/")
...
def event_visible(evt, user_id):
    """Live event stream isolation: a user sees their own session-tagged
    events plus untagged boot/system events only - never another user's
    activity (record ids, routed task text, etc)."""
    session = evt.get("session")
    if session is not None:
        return session == user_id
    return evt.get("cat") in ("boot",)
