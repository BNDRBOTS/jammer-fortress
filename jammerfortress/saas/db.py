...
CREATE TABLE IF NOT EXISTS stripe_events (
  event_id TEXT PRIMARY KEY, received REAL NOT NULL);
...
class DB:
    ...
    def mark_event(self, event_id):
        """Record a Stripe event id. True the first time, False on replay."""
        with self._conn() as c:
            cur = c.execute("INSERT OR IGNORE INTO stripe_events (event_id, received) "
                            "VALUES (?,?)", (event_id, time.time()))
