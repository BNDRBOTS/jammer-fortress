...
def t_embed_backend_registry():
    ...
    inf = E.info()
    _assert(inf["backend"] in ("hash", "syn", "onnx"), f"unknown backend: {inf}")
    prev = E.backend()
    try:
...
# ------------------------------------------------- hardening fixes (v2.1)
def t_syn_backend():
    from . import embeddings as E
    from . import thesaurus
    _assert(thesaurus.available(), "bundled thesaurus missing from package data")
    syn = E.SynBackend()
    a = "the lawyer kept the custody file"
    b = "the attorney retained the custody file"
    far = "banana smoothie recipe blender"
    syn_close = E.cosine(syn.embed(a), syn.embed(b))
    syn_far = E.cosine(syn.embed(a), syn.embed(far))
    hash_close = E.cosine(E.HashBackend().embed(a), E.HashBackend().embed(b))
    _assert(syn_close > 0.7, f"paraphrase similarity too weak: {syn_close:.3f}")
    _assert(syn_far < 0.2, f"unrelated text scored too high: {syn_far:.3f}")
    _assert(syn_close > hash_close + 0.1,
            f"syn backend no better than hash ({syn_close:.3f} vs {hash_close:.3f})")
    _assert(syn.embed("stable point") == syn.embed("stable point"),
            "syn backend lost determinism")


def t_oblivion_paraphrase():
    from . import embeddings as E
    prev = E.backend()
    try:
        E.set_backend(E.SynBackend())
        m = MemoryMesh()
        m.record("the attorney retained the custody file")
        m.record("the lawyer did not keep the custody file")
        contra = OblivionMirror().contradictions(m.snapshot())
        _assert(contra, "reworded contradiction missed by syn backend")
        _assert(contra[0]["shared_anchor"], "contradiction reported without anchors")
    finally:
        E.set_backend(prev)


def t_mesh_quota():
    m = MemoryMesh(max_capacity=100, max_per_session=3)
    for i in range(5):
        m.record(f"user a fact number {i}", session_id="ua")
    m.record("user b lone fact", session_id="ub")
    a_texts = [e["text"] for e in m.snapshot(session_id="ua")]
    b_texts = [e["text"] for e in m.snapshot(session_id="ub")]
    _assert(len(a_texts) == 3 and "user a fact number 4" in a_texts
            and "user a fact number 0" not in a_texts, f"quota eviction wrong: {a_texts}")
    _assert(b_texts == ["user b lone fact"], f"quota crossed user boundary: {b_texts}")
    _assert(m.query("user a fact", top_k=5, session_id="ua"),
            "query broken after quota eviction")


def t_db_replay():
    with tempfile.TemporaryDirectory() as td:
        db = DB(os.path.join(td, "t.db"))
        _assert(db.mark_event("evt_1") is True, "first event rejected")
        _assert(db.mark_event("evt_1") is False, "replayed event accepted")
        _assert(db.mark_event("evt_2") is True, "distinct event rejected")


def t_ws_visibility():
    from .saas.server import event_visible
    _assert(event_visible({"cat": "record", "session": "u1"}, "u1"), "own event hidden")
    _assert(not event_visible({"cat": "record", "session": "u1"}, "u2"),
            "cross-user event leaked")
    _assert(not event_visible({"cat": "route", "session": "u1",
                               "payload": {"task": "private text"}}, "u2"),
            "cross-user task text leaked")
    _assert(event_visible({"cat": "boot"}, "u2"), "system boot event hidden")
    _assert(not event_visible({"cat": "token_issued"}, "u2"),
            "untagged non-boot event leaked")


def t_public_url():
    from .saas.server import compute_base_url
    saved = os.environ.pop("JF_PUBLIC_URL", None)
    try:
        _assert(compute_base_url({"Host": "h.example", "X-Forwarded-Proto": "https"})
                == "https://h.example", "proxy fallback broken")
        os.environ["JF_PUBLIC_URL"] = "https://fortress.bndr.llc/"
        _assert(compute_base_url({"Host": "evil.example"}) == "https://fortress.bndr.llc",
                "JF_PUBLIC_URL not enforced over Host header")
    finally:
        os.environ.pop("JF_PUBLIC_URL", None)
        if saved is not None:
            os.environ["JF_PUBLIC_URL"] = saved
...
def t_server_e2e():
    ...
    try:
        ...
        # replayed webhook (same Stripe event id) must be ignored, not re-applied
        event_r = json.dumps({"id": "evt_e2e_replay", "type": "customer.subscription.updated",
                              "data": {"object": {"id": "sub_e2e", "status": "active"}}}).encode()
        sig_r = billing.sign_payload(event_r, "whsec_e2e")
        code, r = _http("POST", f"{base}/api/billing/webhook", event_r,
                        headers={"Stripe-Signature": sig_r})
        _assert(code == 200 and r["action"] != "replay ignored", f"first delivery misflagged: {r}")
        code, r = _http("POST", f"{base}/api/billing/webhook", event_r,
                        headers={"Stripe-Signature": sig_r})
        _assert(code == 200 and r["action"] == "replay ignored", f"replayed webhook re-applied: {r}")

        # password spray: 5 misses lock that ip+email pair out with 429
        spray = [_http("POST", f"{base}/api/login",
                       {"email": "ghost@bndr.llc", "password": "wrong-pass-123"})[0]
                 for _ in range(6)]
        _assert(spray[:5] == [401] * 5 and spray[5] == 429, f"login throttle wrong: {spray}")
...
def run():
    ...
    tests = [
        ...
        ("embeddings: syn backend paraphrase separation (bundled thesaurus)", t_syn_backend),
        ("L3 oblivion: reworded contradiction caught via synonym bridge", t_oblivion_paraphrase),
        ("mesh: per-user quota evicts own oldest, never neighbors", t_mesh_quota),
        ("db: stripe event replay guard blocks duplicates", t_db_replay),
        ("server: ws event stream per-user visibility rules", t_ws_visibility),
        ("server: JF_PUBLIC_URL pins billing redirect base url", t_public_url),
